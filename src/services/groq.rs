...
pub fn get_file_contents(owner: &str, repo: &str, commit_sha: &str) -> Vec<(String, String)> {
  ...
  for file in changed_files.iter() {
    ...
    let response = client.get(&file_url).header("User-Agent", "rust-client").send();
...
pub fn extract_new_functions(owner: &str, repo: &str, commit_sha: &str) {
  ...
  let response = client
    ...
    .header(
      "Authorization",
      format!("Bearer {}", "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"),
    )
    .json(&json!({
  ...
  let extracted_text =
    response_json["choices"].get(0).and_then(|choice| choice["message"]["content"].as_str());
  ...
  let json_end =
    extracted_text.find("</functions>").map(|i| i + 11).unwrap_or(extracted_text.len());
